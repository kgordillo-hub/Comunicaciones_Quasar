...
El ejercicio, básicamente plantea un problema de [trilateración](https://es.wikipedia.org/wiki/Trilateraci%C3%B3n). Es decir, determinar la posición relativa de un objeto dadas las coordenadas de otros tres elementos y, sus distancias (radios) a dicho objeto. En este caso, las coordenadas del elemento a calcular son de una nave de la alianza rebelde y, los objetos de referencia para llevar a cabo la trilateración son satélites de la alianza rebelde.

Se define que la nave espacial enviará a cada uno de los satélites un mensaje y su distancia relativa a este. A continuación, se enumeran ciertas reglas de negocio que se consideraron pertinentes para abordar el problema.
...
2. La palabra del mensaje no puede ser vacía en la misma posición para todos los mensajes recibidos. Por ejemplo, s1 recibe: "","es","un","mensaje","". Sí s2 recibe: "","","un","","secreto". s3 recibe: "","","un","","". Aunque todos tengan la misma longitud, en la posición 1 de todos los mensajes la palabra está vacía ("").
3. La palabra del mensaje no puede ser diferente en la misma posición (si no es vacía) para todos los mensajes recibidos. Por ejemplo, s1 recibe: "hola","es","un","mensaje","". 22 recibe: "este","","un","","secreto". s3 recibe: "","","un","","". Aunque todos tengan la misma longitud, en la posición 1 de los mensajes 1 y 2, la palabra es diferente: 'hola' y 'este', respectivamente.
...
En el siguiente diagrama se ilustran las funcionalidades a las que tiene acceso el usuario (nave rebelde):
...
Para el despliegue, se utilizó una espacion de ejecución compartido de Azure.
...
Como se observa, se logró un 92% del cubrimiento del código. Sobre el 8% restante, corresponde a casos de prueba que involucran operaciones de IO (lectura y escritura en disco) y que por su naturaleza son complicados de replicar utilizando Spring testing.

## Servicio web

**Para el despliegue, como se utilizó un espacio de ejecución compartido, puede que algunas peticiones tarden en procesarse. Sin embargo, se puede realizar un escalado horizontal o vertical de la aplicación en cualquier momento, según la demanda.**

La aplicación fue desplegada en Azure, a continuación se presentan los 'endpoint':

### Endpoints

Endpoint de auto descripción del servicio. En este mismo endpoint se pueden realizar pruebas y se encuentra la descripción del API:
```
...
A continuación, se presenta una tabla con los códigos HTTP y los mensajes de respuesta a las diferentes peticiones. Si existe alguna violación a una regla de negocio, se retornará un código de error 404 (not found) con alguno de los mensajes mencionados:
